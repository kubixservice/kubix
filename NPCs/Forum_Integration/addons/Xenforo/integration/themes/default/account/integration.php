...
{
	$sql = "select username from `xf_user` WHERE `email`='".$forumMail."'";
...
{
	$sql = "select email from `xf_user` WHERE `username`='".$forumName."'";
	$sth  = $server->connection->getStatement($sql);
	$sth->execute();
...
if(isset($_POST['forum']))
{
	...
	$postMessage = "
		...
		Confirm your email: ".Flux::message('IntegrationServerURL')."?module=account&action=forumactivate&code=".$rand."&user=".$forumName."
	";
	$headers = "From: ".Flux::message('IntegrationServerName')." <".Flux::message('IntegrationServerMail').">\r\n"; 
	$headers .= "Bcc: ".Flux::message('IntegrationServerMail')."\r\n"; 
